...
public class InsertShop extends HttpServlet {
	...
	protected void service(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException{
		...
		if (shopname == null || shopname.length() == 0) {
			out.println("<script>alert('请输入店铺名');window.history.back();</script>");
		} else if (password == null || password.length() == 0) {
			out.println("<script>alert('请输入密码');window.history.back();</script>");
		}else if (address == null || address.length() == 0) {
			out.println("<script>alert('请输入店铺地址');window.history.back();</script>");
		}else if (phone == null || phone.length() == 0) {
			out.println("<script>alert('请输入电话');window.history.back();</script>");
		}else if (classes == null || classes.length() == 0) {
			out.println("<script>alert('请输入店铺分类');window.history.back();</script>");
		}
                         ...
                         try{
                          ...
                          String sql="INSERT INTO shop (shopname,password,address,phone,classes) VALUES('"+shopname+"','"+password+"','"+address+"','"+phone+"','"+classes+"')";
