		...
		if (username == null || username.length() == 0) {
			...
			out.println("<script>alert('请输入电话');window.history.back();</script>");
                         ...
                         try{
                          ...
                          String sql="UPDATE user SET username='"+username+"',email='"+email+"',phone='"+phone+"',password='"+password+"' WHERE uid="+userid;
                          int resultSet = statement.executeUpdate(sql);//执行sql，获取结果集
                          if(resultSet>0){
                          
                                  out.println("<script>alert('更新成功');window.location.href='web/admin/indexs.jsp';</script>");
