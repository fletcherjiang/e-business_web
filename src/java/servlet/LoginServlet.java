			...
			out.println("<script>alert('请输入密码');window.history.back();</script>");
                   ...
                   // out.println("1111");
                         try{
                          ...
                          String sql="select * from admin where admin ="+name;
                          ResultSet resultSet = statement.executeQuery(sql);//执行sql，获取结果集
                          if(resultSet.next()){
                          
                                  if(password.equals(resultSet.getString("admin"))){
                                      // 获取session
                                        HttpSession hs = request.getSession();
                                        hs.setAttribute("admin",name);
                                      out.println("<script>alert('登录成功');window.location.href='web/admin/indexs.jsp';</script>");
                                     }
                                else{
